...
export default function AdminKB() {
  ...
  const loadKB = async () => {
    try {
      ...
      showMessage('Chyba připojení: ' + e.message, 'error');
    }
  };
  ...
  const addNews = () => {
    ...
    const newsArray = Array.isArray(kb.news) ? [...kb.news] : [];
    newsArray.push({
  ...
  const updateNews = (idx, field, value) => {
    if (!Array.isArray(kb.news)) return;
  ...
  const addFaq = () => {
    ...
    const faqArray = Array.isArray(kb.faq) ? [...kb.faq] : [];
    faqArray.push({ q: newFaq.q, a: newFaq.a });
    setKB({ ...kb, faq: faqArray });
    setNewFaq({ q: '', a: '' });
    setHasChanges(true);
  ...
  const updateFaq = (idx, field, value) => {
    if (!Array.isArray(kb.faq)) return;
  ...
  const scanWebsite = async () => {
    ...
    try {
      ...
      if (data.success && data.kb) {
        ...
        if (saveRes.ok) {
          // DŮLEŽITÉ: Refreshni KB v panelu!
          setKB(data.kb);
          showMessage(`Chat naskenoval web! Nalezeno ${data.extracted || 0} novinek!`);
        }
      } else {
  ...
  if (!kb) return <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'system-ui, sans-serif' }}>Nacitam...</div>;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', borderBottom: '2px solid #e5e7eb', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setActiveTab('news')} style={styles.tab(activeTab === 'news')}>Novinky ({Array.isArray(kb.news) ? kb.news.length : 0})</button>
        ...
        <button onClick={() => setActiveTab('faq')} style={styles.tab(activeTab === 'faq')}>FAQ ({Array.isArray(kb.faq) ? kb.faq.length : 0})</button>
      ...
      {activeTab === 'news' && (
        <div>
          ...
          <h3>Existujici novinky ({Array.isArray(kb.news) ? kb.news.length : 0})</h3>
          {!Array.isArray(kb.news) || kb.news.length === 0 ? (
      ...
      {activeTab === 'faq' && (
        <div style={styles.card}>
          ...
          <h3 style={{ marginTop: '30px' }}>Existujici FAQ ({Array.isArray(kb.faq) ? kb.faq.length : 0})</h3>
          {!Array.isArray(kb.faq) || kb.faq.length === 0 ? (
            <div style={{ textAlign: 'center', color: '#999', padding: '20px' }}>Zatim zadne FAQ</div>
          ) : (
            kb.faq.map((item, idx) => (
